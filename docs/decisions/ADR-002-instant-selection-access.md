...
Status: **Accepted and implementation validated locally; amended on 10 September 2026 for explicit magic-icon activation**
...
- During onboarding, explain Selection Magic before requesting access.
...
- When the approved origins change, update or replace that registration. When access is revoked or Selection Magic is disabled, unregister it.
...
Users receive a deliberate two-step experience—select, then click the magic icon—and Chrome must show a site-access permission prompt so the icon can appear immediately. Optional runtime permission gives users context and control, although all-site access remains powerful and requires precise store disclosure. The extension needs both a lightweight selection observer and robust icon and overlay-positioning tests.
