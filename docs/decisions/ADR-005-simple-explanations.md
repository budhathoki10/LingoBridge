# ADR-005: Simple explanations with NVIDIA Nemotron 3 Super

Status: **Accepted; implemented and verified locally with the fake gateway; live NVIDIA answers not yet checked**
Decision date: **15 September 2026** (Claude Haiku 4.5 was chosen first the same day and replaced by Nemotron 3 Super before release)
...
whether the phrase sounds formal or casual, a usage note, and two example sentences.
...
- Add a gateway route, `POST /v1/explain`, backed by **NVIDIA Nemotron 3 Super**
  (`nvidia/nemotron-3-super-120b-a12b`) through the same NVIDIA API catalog endpoint and
...
- **Explain, do not re-translate.** The prompt has the model explain the idea in everyday words for
  ...
  still mostly repeats the translation, the gateway asks once more for simpler, different words
  and keeps the first answer if the rewrite is unusable. This can add one extra model call.
...
- Explanations have their own deadline (`LINGOBRIDGE_EXPLANATION_TIMEOUT_MS`, default 30 seconds)
...
- The privacy notice names Nemotron 3 Super; the store listing and privacy policy must match.
