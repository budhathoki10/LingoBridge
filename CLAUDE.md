...
Phase 0 through Phase 5 implementation and automated verification are complete locally. Phase 6 is next only after explicit authorization. Follow the serial gates in `docs/07-build-plan.md`; credentialed provider smoke tests remain deployment evidence and credentials must never enter the repository.
...
- preserve the Selection Magic access, explicit-click, and consent boundary in `docs/decisions/ADR-002-instant-selection-access.md`;
