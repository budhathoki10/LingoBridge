...
Phase 0 through Phase 3.1 are complete. Phase 3.2, gateway security controls, is the next permitted change after explicit authorization. Work serially from `docs/07-build-plan.md`, complete each exit condition, and do not add real provider calls or credentials before the Phase 3.2 security gate passes.
...
- LingoBridge is a user-triggered multilingual translator. Online mode exposes the current Google Cloud Translation language catalogue; Nepali receives additional evaluation but is not the only supported language.
...
- In Online mode, Google Cloud Translation is primary. NVIDIA `riva-translate-4b-instruct-v2` is a backup only for its documented language pairs; it is not a Nepali fallback.
...
- Never switch from Google to NVIDIA unless the user has accepted the disclosed multi-provider Online mode. Always report which provider produced the result.
